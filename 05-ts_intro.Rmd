...
dice<-runif(1000,1,6)
dice<-round(dice,digits=0)
dice<-data.frame(dice)
dice<-dice%>%mutate(t=seq(1,1000,by=1))
ggplot(dice,aes(x=t,y=dice))+geom_area()+theme_classic()+labs(y="Dado")
```
...
df<-data.frame(e,t,Y1,Y2,Y3,Y4,Y5,Y6,Y7,Y8)
df<-df%>%filter(t>10)%>%as_tsibble(index=t)
...
```{r}
Y1.plot<-ggplot(df,aes(x=t,y=Y1))+geom_line()+geom_hline(yintercept=mean(df$Y1),color="red",linetype="dashed")
